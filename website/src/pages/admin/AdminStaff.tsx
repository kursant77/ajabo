...
import { User, CheckCircle, Plus, ChefHat, HandPlatter, GlassWater, Trash2 } from "lucide-react";
...
const roleConfig: Record<string, { title: string; icon: any; addLabel: string; emptyLabel: string }> = {
    cooks: {
        title: "Oshpazlar",
        icon: ChefHat,
        addLabel: "Oshpaz qo'shish",
        emptyLabel: "Hozircha oshpazlar yo'q",
    },
    waiters: {
        title: "Afitsiantlar",
        icon: HandPlatter,
        addLabel: "Afitsiant qo'shish",
        emptyLabel: "Hozircha afitsiantlar yo'q",
    },
    barmen: {
        title: "Barmenlar",
        icon: GlassWater,
        addLabel: "Barmen qo'shish",
        emptyLabel: "Hozircha barmenlar yo'q",
    },
};

const AdminStaff = () => {
    const { role = "cooks" } = useParams<{ role: string }>();
    const config = roleConfig[role] || roleConfig.cooks;

    const { personnel, loading, addPerson, deletePerson } = useSupabaseStaff(role);
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-card rounded-2xl border border-border/50 p-6 shadow-sm">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-sm text-muted-foreground">Jami {config.title.toLowerCase()}</p>
            ...
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold">{config.title} ro'yxati</h2>
            ...
            <div className="hidden md:block bg-card rounded-2xl border border-border/50 shadow-sm overflow-hidden">
                <div className="grid grid-cols-12 gap-4 p-4 bg-muted/50 font-medium text-sm text-muted-foreground">
                    <div className="col-span-6">Ism</div>
